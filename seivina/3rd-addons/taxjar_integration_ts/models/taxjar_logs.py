        ...
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                if 'company_id' in vals:
                    vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('taxjar.logs') or _('New')
                else:
                    vals['name'] = self.env['ir.sequence'].next_by_code('taxjar.logs') or _('New')
        return super(TaxJarLog, self).create(vals_list)