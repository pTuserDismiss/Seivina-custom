    ...
    def get_views(self, views, options=None):
        ...
        if view_ref.get('views'):
            ...
            hidden_obj = self.env['model.access'].sudo().search(
                [('access_user_management_id', 'in', self.env.user.access_user_management_id.ids),
    ...
    def get_view(self, view_id=None, view_type='form', **options):
        ...
        if view_type == 'tree':
            is_export_hide = self.env['model.access'].sudo().search(
                [('access_model_id.model', '=', view_ref['model']), ('access_user_management_id.active', '=', True),
                 ('access_user_management_id.access_company_ids', 'in', lst),
        ...
        if view_type == 'kanban':
            ...
            for button in hide_button_ids:
                ...
                for link in button.access_kanban_button_ids:
                    ...
                    elif link.access_button_type == 'open':
        ...
        if readonly_access_id:
            ...
            change_model_access = self.env['model.access'].sudo().search([
                ('access_user_management_id.access_user_ids', 'in', self.env.user.id),
                ('access_user_management_id.active', '=', True),
                ('access_model_id.model', '=', view_ref['model']), ('access_user_management_id.access_company_ids', 'in', lst)])
    ...
    def access_hide_field(self, doc, view_ref):
        ...
        for hide_field in hidden_fields:
            for field_id in hide_field.access_field_id:
                ...
                if element:
                    for ele in element:
                        if ele.get('name') == field_id.name or (ele.tag == 'label' and 'for' in ele.attrib.keys() and ele.attrib['for'] == field_id.name) and hide_field.access_field_invisible:
                            ele.set('invisible', '1')
        view_ref['arch'] = etree.tostring(doc, encoding='unicode')
    ...
    def access_hide_filter_groupby(self, doc):
        ...
        hide_filter_ids = self.env['filter.group.access'].sudo().search(
            [('access_model_id.model', '=', self._name),
             ('access_user_management_id.active', '=', True),
             ('access_user_management_id.access_user_ids', 'in', self._uid),
             ('access_user_management_id.access_company_ids', 'in',
              company_lst)])
