...
class KsAccessManagement(models.Model):
    ...
    access_model_access_line = fields.One2many('model.access', 'access_user_management_id', string='Model Access')
    access_hide_field_line = fields.One2many('field.access', 'access_user_management_id', string='Field Access')
    access_domain_access_line = fields.One2many('domain.access', 'access_user_management_id', string='Domain Access')
    access_button_tab_access_line = fields.One2many('button.tab.access', 'access_user_management_id', string='Button Access')
    access_users_count = fields.Integer(string='Users Count', compute='_compute_users_count')
    access_hide_filters_groups_line = fields.One2many('filter.group.access', 'access_user_management_id', string='Filter Group')
    ...
    @api.constrains('access_company_ids')
        ...
        if vals.get('access_user_ids'):
            for domain in self.access_domain_access_line:
                users = self.env['res.users'].sudo().search(
                    [('access_user_management_id', '=', self.id),
    ...
    def access_activate_rule(self):
        ...
        self.active = True
        users = self.env['res.users'].sudo().search(
            [('access_user_management_id', '=', self.id), ('access_user_management_id.active', '=', True)])
        for domain in self.access_domain_access_line:
    ...
    def access_search_action_button(self, model):
        ...
        is_archive_hide = self.env['model.access'].sudo().search(
            [('access_model_id.model', '=', model), ('access_user_management_id.active', '=', True),
             ('access_user_management_id.access_company_ids', 'in', lst),
             ('access_user_management_id.access_user_ids', 'in', self.env.user.id), ('access_hide_archive_unarchive', '=', True)], limit=1)
        is_export_hide = self.env['model.access'].sudo().search(
            [('access_model_id.model', '=', model), ('access_user_management_id.active', '=', True),
             ('access_user_management_id.access_company_ids', 'in', lst),
             ('access_user_management_id.access_user_ids', 'in', self.env.user.id), ('access_hide_export', '=', True)], limit=1)
        if is_archive_hide:
