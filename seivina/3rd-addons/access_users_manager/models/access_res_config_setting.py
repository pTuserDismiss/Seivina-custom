...
class GeneralSettings(models.TransientModel):
    ...
    global_tracking_enable = fields.Boolean(string="Global Tracking", config_parameter='access_users_manager.global_tracking_enable')
...
class IrUiMenu(models.Model):
    ...
    def _load_menus_blacklist(self):
        ...
        active_direct_sales = self.env['ir.config_parameter'].sudo().get_param('access_users_manager.global_tracking_enable')
        if not active_direct_sales:
            all_menus.append(self.env.ref('access_users_manager.menu_global_tracking_first_form').id)
        return all_menus